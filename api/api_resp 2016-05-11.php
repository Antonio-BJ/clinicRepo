...
if($funcion == 'registra'){
    ...
    $sql="Insert into Expediente (
                        ...
                        subPro_clave,
                        Exp_estatusReg)
            VALUES(:folio,:prefijo,:cons,:uni_clave,:usu_registro,:apPat,:apMat,:nombre,:nomComp,:fecNa,:fecNa1,:edad,:meses,:mail,:tipoTel,:telefono,:codPostal,:Exp_fecreg,:ciaClave,:proClave,:Uni_claveActual,:Exp_modificador,:CON_cve,:Exp_cveMemMV,:subPro_clave,:claveEtapa)";
                            ...
                            <table style="font-family: sans-serif; margin: auto;">
                                ...
                                $contenido.=' <tr>
                                    <td colspan="6" style="text-align: justify; font-size: 18px; font-family: sans-serif">
                                        <p>
                                        Actualmente existen '.$cont.' pacientes registrados a quienes no se les ha capturado NOTA M&Eacute;DICA en la Unidad <b>'.$Unidad.'</b>
                                        </p>
                                    </td>
                                </tr>
                            <!--</table>-->                            
...
if($funcion == 'registraSin'){
    ...
    $query = "select Cia_clave, LOC_claveint, Uni_propia from Expediente 
              inner join Unidad on Expediente.Uni_clave = Unidad.Uni_clave  
              where Exp_folio='".$fol."'";
    $result=$conexion->query($query);
    $ciaVal = $result->fetch();
...
if($funcion == 'registraConvenio'){
    ...
    $sql = "INSERT INTO ObsParticulares(Exp_folio, Par_tipo, Par_tipoNom, Par_fecreg, Par_convenio,Par_refNombre, Par_refCedula)
                values(:Exp_folio,4,'convenio',now(),:Par_convenio,:Par_refNombre, :Par_refCedula)"; 
            $stmt = $conexion->prepare($sql);                                                          
            $stmt->bindParam('Exp_folio', $fol);
...
if($funcion == 'SelectItems'){
    ...
    if($cia['Cia_clave']==54){
        if($cia['Exp_modificador']=='T. INB'){
            ...
            $campo='keken';
        }elseif($cia['Exp_modificador']=='ASISS'){
            $campo='assis';
        }elseif($cia['Exp_modificador']=='T. MUL'){
            $campo='T_multiva';
        }
...
if($funcion == 'SelectItems1'){
    ...
    $query="select ItemOrtho_nuevo.ite_cons, ite_item, ite_precio,individual from ItemOrtho_nuevo 
            inner join PermisosItems on ItemOrtho_nuevo.ite_cons = PermisosItems.Ite_cons
            where ite_activo='S' and Tip_clave=".$cveFam." and individual<>'N' order by ite_orden;";
    $result = $db->query($query);
    $tipoItem = $result->fetchAll(PDO::FETCH_OBJ);
    echo json_encode($tipoItem);
    $db = null;    
...
if($funcion=='guardaDiagnostico'){
 $fol=$_GET['fol'];
    ...
    try{    
            ...
            $query="Update ObsNotaMed Set ObsNot_diagnosticoRx=:ObsNot_diagnosticoRx,ObsNot_obs=:ObsNot_obs, ObsNot_glasgow=:ObsNot_glasgow WHERE Exp_folio=:Exp_folio";                               
            $temporal = $db->prepare($query);
            $temporal->bindParam("Exp_folio", $fol);
...
if($funcion=='guardaLesionAdministrativa'){
    ...
    try{           
            $query="Update ObsNotaMed Set ObsNot_tipoLesion=:ObsNot_tipoLesion,Clave_lesionMV=:Clave_lesionMV,Clave_lesionCia=:Clave_lesionCia, TAD_importe=:TAD_importe WHERE Exp_folio=:Exp_folio";                               
            $temporal = $db->prepare($query);
            $temporal->bindParam("Exp_folio", $fol);                       
            $temporal->bindParam("ObsNot_tipoLesion", $tipoLesion);                 
...
if($funcion=='enviaDatosCancelacion'){
    ...
    try{
        ...
        if($documentos=='NO'){
            ...
            $query="update Expediente SET Exp_cancelado=1, Usu_cancelado='sistema', Exp_fcancelado = now(), Exp_mcancelado=:Exp_mcancelado, Exp_duplicado=:Exp_duplicado where Exp_folio=:Exp_folio";
