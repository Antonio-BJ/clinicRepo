...

{
    ...
    {   
      ...
      $sql = "select Uni_nombreCorto,Unidad.Uni_clave from Expediente
        ...
        $serie    = $rs['Recibo_serie'];
        $identificador = $rs['Recibo_identificador'];
        
        $precioUnitario= $rs['Recibo_total'];
        if(!$identificador){
          $ide = $this->generateRandomString();
          $identificador=$serie.$folRecibo.$uniClave.round($precioUnitario).$ide;
        }
        ...
        $nomArchivo="MEDICA_".$identificador."_".date("Y-m-d").".tck";
        $file=fopen("pruebas/".$nomArchivo,"a") or die("Problemas");
          //vamos añadiendo el contenido
          
          fputs($file,"T|".$identificador."|MedicaVial|".$uniClave."|".$unidad."|".date("Y-m-d H:i:s.u")."|1|3.3||0|".$precioUnitario."|".round($descuentoTotal,2)."|Pago(s)|MXN|1|F");
          ...
          if($mPago==2||$mPago==3||$mpago==6){
              fputs($file,"P|".$identificador."||1|".$terminacion."||||".$precioUnitario."||".$precioUnitario."||".$pago);          
          }else{
              fputs($file,"P|".$identificador."||1|||||".$precioUnitario."||".$precioUnitario."||".$pago);          
          }
          // fputs($file,"\n");
          $descuentoTotal =0; 
          // $rs = $result->fetchAll(PDO::FETCH_ASSOC);
          ...
          foreach ($rs as $key) {
                   ...
                   $imporIvaDesc1 = $importeItem1-$descuentoSinIva+$ivaN;
                    ...
                    fputs($file,"M|".$identificador."||01|||1|".$importeItem."|".$importeItem."|".round($descuentoSinIva,2)."|".round($imporIvaDesc1,2)."||".$key['it_prod']."|IVA:0.16:".round($ivaN,2)."|85121502|H87");                    
            ...
            // fputs($file,"\n");
            fputs($file,"I|".$identificador."||01|||0.16||".round($ivaFinal,2)."|0.00|".round($ivaFinal,2)."|0.00|IVA");
            fputs($file,"\n"); 
            ...
            $sql1 = "UPDATE reciboParticulares SET Recibo_identificador='".$identificador."' WHERE Exp_folio='".$folio."'  and Recibo_cont=".$folioRecibo." and Recibo_serie='".$serie."'";
            $this->_db->query($sql1);
            }catch(Exception $e){
              echo $e->getMessage();
            }
