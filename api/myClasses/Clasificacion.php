...
class Clasificacion extends Modelo
{
    ...
    {        
    	try{
            $sql="select Uni_clave, UNI_nombreMV as Uni_nombrecorto from Unidad 
    ...
    public function verificaUnidad($unidad)
    {        
        try{
    ...
    {         
        ...
        try{
          ...
          $query1 = "Select ";
            ...
            if($fechaIni!='' && $fechaFin!=''){
                //$query.= "Exp_fecreg Between '".$fechaIni." 00:00:00' and '".$fechaFin." 23:59:59' and ";
                $query1.= "Exp_fecreg Between '".$fechaIni." ' '".$horaIni.":00' and '".$fechaFin." ' '".$horaFin.":59' and ";
            }            
          ...
          $query2 = "Select ";
            ...
            $query2.="Where ";
            if($fechaIni!='' && $fechaFin!=''){
                $query2.= "Not_fechareg Between '".$fechaIni." ' '".$horaIni.":00' and '".$fechaFin." ' '".$horaFin.":59' and ";
            }            
          ...
          $query3 = "Select ";
            ...
            if($unidad>=1){$query3.="  Subsecuencia.Uni_clave='".$unidad."' and Exp_cancelado<>1 ";}
          ...
          $query4 = "Select ";
            ...
            $query4.="Where ";
            if($fechaIni!='' && $fechaFin!=''){
                $query4.= "Rehab_fecha Between '".$fechaIni." ' '".$horaIni.":00' and '".$fechaFin." ' '".$horaFin.":59' and ";
            }            
 ...
 public function buscaParametrosReporteRegistro($datos,$uni)
    {         
        $fechaIni=      $datos->fechaIni;
        $fechaFin=      $datos->fechaFin;
        $horaIni=       $datos->horaIni;
        ...
        try{
          ...
          $query1 = "Select";
                $query1.=" Exp_fecreg ";                
                $query1.=", Exp_completo ";               
                $query1.=", Expediente.Exp_folio ";                
                $query1.=", Exp_edad";
                $query1.=", Exp_codPostal";
                $query1.=", ObsNot_diagnosticoRx ";
                $query1.=", NS_descripcion as DESCRIPCION ";
                $query1.=", NS_posologia as INDICACIONES ";
            $query1.= " From Expediente 
                       Inner Join Compania On Expediente.Cia_clave=Compania.Cia_clave 
                       inner Join RecetaMedica On Expediente.Exp_folio=RecetaMedica.Exp_folio
                       inner join NotaSuministros on RecetaMedica.id_receta = NotaSuministros.id_receta
            ...
            $query1.=" GROUP BY Expediente.Exp_folio order by Exp_folio ";
 ...
 public function getReceta($folio)
    {         
        try{
          #----------------- CONSULTA ORIGINAL -----------------#
          $query1 = "SELECT * from RecetaMedica
                    inner join NotaSuministros on RecetaMedica.id_receta = NotaSuministros.id_receta
    ...
    public function buscaParametrosReporteParticulares($datos,$uni)
    {         
        $fechaIni=      $datos->fechaIni;
        $fechaFin=      $datos->fechaFin;
        $horaIni=       $datos->horaIni;
        ...
        try{
            ...
            $query.= "From Expediente Inner Join Compania On Expediente.Cia_clave=Compania.Cia_clave 
            left join Convenio on Expediente.CON_cve=Convenio.CON_cve ";
            $query.="Where ";
            if($fechaIni!='' && $fechaFin!=''){
                //$query.= "Exp_fecreg Between '".$fechaIni." 00:00:00' and '".$fechaFin." 23:59:59' and ";
                $query.= "Exp_fecreg between '".$fechaIni." ".$horaIni."' and '".$fechaFin." ".$horaFin."' and ";
            }            
            $query.="  Expediente.Cia_clave in (51,54) and";            
            if($unidad>=1){$query.="  Uni_clave='".$unidad."' and Exp_cancelado<>1 ";}
            else{$query.=" Uni_clave='".$uni."' and Exp_cancelado<>1 ";}
            $query.=" order by Exp_folio";                        
...
//buscamos particulares
    ...
    public function listadoItemsParticulares()
    {       
        $uni = $_COOKIE["uniClave"];
        try{
            $query="SELECT ItemOrtho_nuevo.Ite_cons, Ite_item, ITE_descripcion, ITE_precio".$uni." ITE_precio,Ite_precio as Publico,  M_MV, Tip_nombre, Ite_img FROM ItemOrtho_nuevo
                    INNER JOIN TipoItem ON ItemOrtho_nuevo.Tip_clave=TipoItem.Tip_clave
                    INNER JOIN PermisosItems ON ItemOrtho_nuevo.Ite_cons = PermisosItems.Ite_cons
                    WHERE ite_activo='S' and M_MV<>'N'
                    ORDER BY Tip_nombre";
            $result = $this->_db->query($query);
            $respuesta = $result->fetchAll(PDO::FETCH_OBJ);    
        }catch(Exception $e){
    ...
    public function getConvenioExp($folio)
    {        
        try{
            $query="Select CON_cve From Expediente where Exp_folio='".$folio."'";
            $result = $this->_db->query($query);
            $respuesta = $result->fetch();    
        }catch(Exception $e){
            //$respuesta=$e->getMessage();
    ...
    public function getAtencionesMedicosGenerales($datos)
    {        
        try{
            $query="Select Expediente.Exp_folio, Exp_completo, Exp_fecreg, Uni_nombrecorto, Cia_nombrecorto, concat(Med_nombre,' ',Med_paterno,' ',Med_materno) medico From Expediente 
            inner join Compania on Expediente.Cia_clave = Compania.Cia_clave
            inner join Unidad on Expediente.Uni_clave = Unidad.Uni_clave
            ...
            where Exp_cancelado<>1 and Med_tipo=2 and Exp_fecreg between '".$datos->fechaIni."' and '".$datos->fechaFin." 23:59:59'
            ";
            $result = $this->_db->query($query);
