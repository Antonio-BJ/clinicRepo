...
class NotaMedica extends Modelo
{
    ...
    {    
        try{                       
            $query ="select NotaSuministros.id_receta AS RECETA,Exp_folio AS FOLIO,RM_fecreg as FECHA_RECETA, count(NotaSuministros.id_receta) AS ITEMS_FALTANTES from NotaSuministros 
                inner join RecetaMedica on NotaSuministros.id_receta = RecetaMedica.id_receta
                WHERE Uni_clave=".$uni." and NS_cancelado<>1 and NS_surtida=0
