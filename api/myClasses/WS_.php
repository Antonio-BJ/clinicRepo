...
try {
	...
	$$xml='
         <gmqXmlRequest>
                <xmlRequest pid="1" token="ad8787769969818cc9f4f9a656298f58">
                                           <fecha_reporte>2018-06-05</fecha_reporte>
                            </xmlRequest>
            </gmqXmlRequest>
      ';
	...
	foreach ($result as $key => $value) {
        ...
        if($datos['contador']>0){
            ...
            $queryUpdate = "UPDATE QualitasWS_resp SET QWS_estatus=".$miArray['STATUS'].", QWS_folioAutorizacion='".$miArray['FOLIO_AUTORIZACION']."', QWS_fechaActualizacion=now(),Exp_folio='".$miArray['FOLIO_PROVEEDOR']."',QWS_riesgo='".$miArray['COBERTURA']."', QWS_observaciones='".$miArray['OBSERVACIONES']."' WHERE QWS_folioElectronico='".$miArray['FOLIO']."'";
