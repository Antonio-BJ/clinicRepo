...
/*****************************************************************************************/
/****************************** listado de folios para reporte   ************************/
if($funcion=='buscaParametrosReporteRegistro'){
    $uni=$_GET['uni'];

    $postdata = file_get_contents("php://input");    
    $datos = json_decode($postdata);     
    $clasificacion = new Clasificacion();
    $busqueda = $clasificacion->buscaParametrosReporteRegistro($datos,$uni);    
    echo json_encode($busqueda);
}
/*****************************************************************************************/
/****************************** listado de folios para reporte   ************************/
if($funcion=='getReceta'){
    $folio=$_GET['folio'];

    $postdata = file_get_contents("php://input");    
    $datos = json_decode($postdata);     
    $clasificacion = new Clasificacion();
    $busqueda = $clasificacion->getReceta($folio);    
    echo json_encode($busqueda);
}
/*****************************************************************************************/
/****************************** listado de folios para reporte   ************************/
if($funcion=='buscaParametrosReporteParticular'){
    $uni=$_GET['uni'];
    $postdata = file_get_contents("php://input");    
    $datos = json_decode($postdata);     
    $clasificacion = new Clasificacion();
    $busqueda = $clasificacion->buscaParametrosReporteParticulares($datos,$uni);    
    echo json_encode($busqueda);
}
/*****************************************************************************************/
...
/****************************** verifica unidades ************************/
if($funcion=='verificaUnidad'){
    $unidad=$_GET['unidad'];
    $postdata = file_get_contents("php://input");    
    $datos = json_decode($postdata);     
    $clasificacion = new Clasificacion();
    $busqueda = $clasificacion->verificaUnidad($datos,$unidad);    
    echo json_encode($busqueda);
}
/*****************************************************************************************/
/****************************** listado de folios sin documentacion   ************************/
if($funcion=='getItemsParticulares'){        
    $clasificacion = new Clasificacion();
    $listadoSinDoc = $clasificacion->listadoItemsParticulares();    
    echo json_encode($listadoSinDoc);
}
/*****************************************************************************************/

/****************************** listado de particulares ************************/
if($funcion=='getConvenioExp'){
    $folio=$_GET['fol'];
    $clasificacion = new Clasificacion();
    $particulares = $clasificacion->getConvenioExp($folio);    
    echo json_encode($particulares);
}
/*****************************************************************************************/

/****************************** medico general ************************/
if($funcion=='AtencionesMedicoGeneral'){
    $postdata = file_get_contents("php://input");    
    $datos = json_decode($postdata);     
    $clasificacion = new Clasificacion();
    $particulares = $clasificacion->getAtencionesMedicosGenerales($datos);    
    echo json_encode($particulares);
}
/*****************************************************************************************/
?>
