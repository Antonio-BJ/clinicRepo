...
*/
       ...
       /* $contenido='<HTML>
                                ...
                                LES HAGO LA CORDIAL INVITACION PARA QUE EL DIA VIERNES ME ACOMPAÑEN A COMER UNOS DELICIOSOS MACHETES
        ...
        $mimemail->set_from("mvseguimiento@medicavial.com.mx");
        ...
        $mimemail->set_subject("INVITACION A COMER");
...
if($funcion == 'validaSubsecuencia'){
    ...
    $listInter = $result->fetch();

    $cont = $listInter['Cons'];

    if($cont==null){

        $query="Select Exp_fecreg as fechaRegistro From Expediente Where Exp_folio='".$fol."'";

        $result = $db->query($query);

        $fechaFolio = $result->fetch();

        $fechaUltima =  new DateTime($fechaFolio['fechaRegistro']);
        ...
        $query="Select Sub_fecha, Sub_hora From Subsecuencia Where Exp_folio='".$fol."' and Sub_cons=".$con;

        $result = $db->query($query);

        $fechaFolio = $result->fetch();

        $fechaUltima = new DateTime($fechaFolio['Sub_fecha'].' '.$fechaFolio['Sub_hora']);

    }

    $intervalo= $fechaUltima->diff($fecha);    

    $datos['dias']= $intervalo->d;

    $datos['horas']= $intervalo->h;

    $datos['minutos']= $intervalo->i;

    $datos['segundos']= $intervalo->s;

    $datos['fechaUltima']= $fechaUltima;

    $datos['fechaHoy']= $fecha;    
...
if($funcion=='listaTiposDigitales'){
    ...
    $query = "SELECT Per_FactExp from Permiso where Usu_login='".$usr."'";

    $per = $conexion->query($query);

    $permiso = $per->fetch();

    $perFactExp = $permiso['Per_FactExp'];

    //$usr=='algo'||$usr=='earobles'||$usr=='mmerino'||$usr=='iarvizu'
...
if($funcion == 'revisaUltimoRegistro'){ 
    ...
    $query="SELECT Exp_folio, Exp_estatusReg 
            ...
            where Uni_clave='".$unidad."' ".$usuario."
