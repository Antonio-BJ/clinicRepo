app.controller('registraCtrl', function($scope,$rootScope, $http,$cookies,$cookieStore,$location,$routeParams,webStorage) {
var hoy = new Date();
var dd = hoy.getDate();
...
$scope.inicio = function(){
    ...
    $scope.buscaOrdenRehabSPT = function(){
        $scope.ordenRhspt.buscando = true;
        $scope.ordenRhspt.datosOrden = null;

        $http.get('http://busqueda.medicavial.net/apiordenesRehaSPT/externos/buscaPase-'+$scope.ordenRhspt.clave).success(function (data){
            console.log(data);
            if ( data.length > 0 ) {
                $scope.ordenRhspt.datosOrden = data;
                $rootScope.cveProd=4
            };
            $scope.ordenRhspt.buscando = false;
        });
    }
    ...
    $scope.ordenXnombreSPT = function(){
        $scope.ordenRhspt.buscando = true;
        $scope.ordenRhspt.datosOrden = null;

        $http({
            url:'http://busqueda.medicavial.net/apiordenesRehaSPT/externos/pasesXnombre',
            method:'POST',
            contentType: 'application/json',
            dataType: "json",
            data: { nombre: $scope.ordenRhspt.nombre }
            }).success( function (data){
                console.log(data);
                if ( data.length > 0 ) {
                    $scope.ordenRhspt.datosOrden = data;
...
$scope.prellenaOrden = function( datos ){
    ...
    $scope.datos.ordensist  = datos.ordensis;
    $scope.datos.claveOrden = datos.claveOrden;

    if(datos.ordensis == 1){

      $('#modalOrdRehab').modal('hide');

    }else if(datos.ordensis == 2){

      $('#modalOrdRehabspt').modal('hide');
...
/* VERIFICA Y CARGA EL ULTIMO REGISTRO SI FUE INCONCLUSO */
    $http({
        ...
        $rootScope.ciaClave=$scope.estadoUltimo[0].Cia_clave;
        console.log($scope.estadoUltimo[0]);
            if ($scope.estadoUltimo!='') {
                ...
                $scope.folio=$scope.estadoUltimo[0].Exp_folio;
                                ...
                                else if($rootScope.ciaClave==51 || $rootScope.ciaClave==81 || $rootScope.ciaClave==76 || $rootScope.ciaClave==83){
...
$scope.guardaEtapa1 = function(){
    ...
    var curp= document.getElementById("inlineradio10").checked;
    var curp1= document.getElementById("inlineradio20").checked;
    var particularMedicoS= document.getElementById("particularMedicoS").checked;
    var particularMedicoN= document.getElementById("particularMedicoN").checked;

    if(curp==false && curp1== false){
        alert("Debe validar el CURP del paciente");
    }else{
        if(particularMedicoS==false && particularMedicoN==false && $rootScope.ciaClave==51){
            alert("Debe especificar si es paciente partciular del m\u00E9dico");
        }else{
                    ...
                    if($scope.aniosValidacion==false){
                        ...
                        $http({
                            ...
                            if(data!='nada'){
                                ...
                                $rootScope.ciaClave=$cookies.clave;
                                $rootScope.cveProd=$cookies.clavePro;
                                console.log($rootScope.ciaClave);
                                console.log($rootScope.cveProd);
                                $http({
...
/////////////////////// TERMINA SELECCION DE REFERENCIAS ////////////////////////////
    ...
    /* termina de agregar los datos de las referencias */
        ...
        $scope.cargador1=true;
        if($scope.obsPart.entero!=0){
        if($scope.obsPart.observacion!=''){
            $scope.rhTipCovid = 0;
            if(webStorage.session.get('rhCovidTipo')){
                $scope.rhTipCovid = webStorage.session.get('rhCovidTipo');    
            }
            console.log("api/api.php?funcion=registraPart&fol="+$scope.folio+"&cveRhCovid="+$scope.rhTipCovid);
            console.log($scope.obsPart);
        $http({
            url:'api/api.php?funcion=registraPart&fol='+$scope.folio+'&cveRhCovid='+$scope.rhTipCovid,
            method:'POST',
            contentType: 'application/json',
            dataType: "json",
            data: $scope.obsPart
            ...
            $scope.cargador1=false;
            $scope.msjcomoSeentero='El campo como se enteró es obligatorio.';
        }
    }
...
/*************************************************  fin nuevo ajustador **************************************/
    ...
    $scope.verificaRadio = function(){
        ...
        if($scope.obsPart.entero==4){
            $scope.ver1=true;
            $scope.ver4=false;
            $scope.ver9=false;
            $scope.ver7=false;
        }else if($scope.obsPart.entero==5){
            $scope.ver4=true;
            $scope.ver1=false;
            $scope.ver9=false;
            $scope.ver7=false;
        }else if($scope.obsPart.entero==6){
            $scope.ver9=true;
            $scope.ver1=false;
            $scope.ver4=false;
            $scope.ver7=false;
        }else if($scope.obsPart.entero==7){
            $scope.ver9=false;
            $scope.ver1=false;
            $scope.ver4=false;
            $scope.ver7=true;
