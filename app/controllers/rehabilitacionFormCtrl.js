...
if ($scope.noR<=1) {
  ...
  $http.get('api/api.php?funcion=verificaHCR&folio='+$rootScope.folio).success(function (data){
      //console.log(data);
      if (data=='false') {
        //console.log('no ha realizado HCR');
        $scope.hcrExistente=false;
      } else{
        //console.log('ya hay HCR registrada');
        $scope.hcrExistente=true;
      };
  });

  $http.get('api/api.php?funcion=getRangos&folio='+$rootScope.folio).success(function (data){                      
      console.log(data);
      if (data=='') {
        $scope.listaRangos=undefined;
      } else {
        $scope.listaRangos=data;
      };
  });

  $http.get('api/api.php?funcion=getRegiones').success(function (data){                      
    // console.log(data);
    $scope.listaRegiones=data;
});
...
*/
  $http.get('api/api.php?funcion=getProducto&folio='+$rootScope.folio).success(function (data){
      ...
      if ( data.Cia_clave =='64' ) {
        $scope.ordenRh = true;
        $scope.buscaOrdReh( $rootScope.folio );
      };

      console.log($scope.ordenRh);
      
      // if (data.Pro_clave=='4' && $rootScope.uniClave=='8') {
      if (data.Pro_clave=='4' || data.Pro_clave=='5') {
...
$scope.buscaOrdReh = function( folio ){
    $http({
        url:'http://busqueda.medicavial.net/api/externos/pasesXfolio',
        method:'POST',
...
/*  $scope.datosHCR={};
  ...
  $http.get('api/api.php?funcion=getDiagnosticoInicial&folio='+$rootScope.folio).success(function (data){
      //console.log(data);
      $scope.histClinicaReh.diagInicial=data.ObsNot_diagnosticoRx;
  });
...
$scope.histClinicaReh={
  ...
  mesesDiabetes:null,
  mesesHipertension:null,
  mesesMarcapasos:null,
  mesesRadioterapia:null,
  mesesEpilepsia:null,
  mesesPielSensible:null,
  mesesLupus:null,
  mesesIntervenciones:null,
  diagInicial: '',
  valInicial:'',
  tratamientosPrevios:'',
  region:null,
              ...
              $http({
                    ...
                    data: $scope.rehabilitacionForm
                      ...
                      if(data.respuesta=='SI'){
                        $http.get('api/convenio.php?funcion=enviarCorreoRH&fol='+$rootScope.folio).success(function (data){
  ...
  $scope.guardarComoLLego = function(){  
    $scope.mensajeLlego=''; 
      $http.get('api/api.php?funcion=comoLlego&fol='+$rootScope.folio+'&llego='+$scope.seEntero).success(function (data){  
      console.log(data);                                              
       if(data=='exito'){
        $('#comoLlegoModal').modal('hide');
       }else{
          $scope.mensajeLlego='ocurrio un problema';
       }
      });                      
  }
...
/*  $scope.agregaEnf = function(){
  ...
  $scope.continuarRh = function(){
    ...
    if ($scope.histClinicaReh.diagInicial=='' || $scope.histClinicaReh.valInicial=='' || $scope.histClinicaReh.tratamientosPrevios=='' ||
        $scope.histClinicaReh.escalaDolor==null || $scope.histClinicaReh.examenMuscular=='' || $scope.histClinicaReh.estadoActual=='' ||
        $scope.listaRangos==undefined || $scope.histClinicaReh.tipoDolor==null || 
        ($scope.histClinicaReh.diabetes==1 && ($scope.histClinicaReh.aniosDiabetes==null && $scope.histClinicaReh.mesesDiabetes==null)) ||
        ($scope.histClinicaReh.hipertension==1 && ($scope.histClinicaReh.aniosHipertension==null && $scope.histClinicaReh.mesesHipertension==null)) ||
        ($scope.histClinicaReh.marcapasos==1 && ($scope.histClinicaReh.aniosMarcapasos==null && $scope.histClinicaReh.mesesMarcapasos==null)) ||
        ($scope.histClinicaReh.radioterapia==1 && ($scope.histClinicaReh.aniosRadioterapia==null && $scope.histClinicaReh.mesesRadioterapia==null)) ||
        ($scope.histClinicaReh.epilepsia==1 && ($scope.histClinicaReh.aniosEpilepsia==null && $scope.histClinicaReh.mesesEpilepsia==null)) ||
        ($scope.histClinicaReh.pielSensible==1 && ($scope.histClinicaReh.aniosPielSensible==null && $scope.histClinicaReh.mesesPielSensible==null)) ||
        ($scope.histClinicaReh.lupus==1 && ($scope.histClinicaReh.aniosLupus==null && $scope.histClinicaReh.mesesLupus==null)) ||
        ($scope.histClinicaReh.intervenciones==1 && ($scope.histClinicaReh.aniosIntervenciones==null && $scope.histClinicaReh.mesesIntervenciones==null))
        )
/*    if ($scope.histClinicaReh.diagInicial=='' || $scope.histClinicaReh.valInicial=='' || $scope.histClinicaReh.tratamientosPrevios=='' ||
        $scope.histClinicaReh.escalaDolor==null || $scope.histClinicaReh.examenMuscular=='' || $scope.histClinicaReh.estadoActual=='' ||
        $scope.listaRangos==undefined || $scope.histClinicaReh.tipoDolor==null)*/
...
/*  $http.get('api/api.php?funcion=getRangos&folio='+$rootScope.folio).success(function (data){
      if (data[0].RM_id==null) {
          $scope.listaRangos=data;
          console.log($scope.listaRangos);
      } else{
          $scope.listaRangos=undefined;
          console.log($scope.listaRangos);
  ...
  $scope.guardaRangos = function(){
    if ($scope.histClinicaReh.region==null || $scope.histClinicaReh.region=='' ||
        ($scope.histClinicaReh.region=='1' && ($scope.histClinicaReh.flexion==null || $scope.histClinicaReh.extension==null || $scope.histClinicaReh.lateralizacion==null || $scope.histClinicaReh.rotaciones==null))
        || ($scope.histClinicaReh.region=='2' && ($scope.histClinicaReh.flexion==null || $scope.histClinicaReh.extension==null))
        || ($scope.histClinicaReh.region=='3' && ($scope.histClinicaReh.inclinaciones==null || $scope.histClinicaReh.rotaciones==null))
        || ($scope.histClinicaReh.region=='4' && ($scope.histClinicaReh.flexion==null || $scope.histClinicaReh.extension==null || $scope.histClinicaReh.abduccion==null || $scope.histClinicaReh.aduccion==null || $scope.histClinicaReh.rotExterna==null || $scope.histClinicaReh.rotInterna==null))
        || ($scope.histClinicaReh.region=='5' && ($scope.histClinicaReh.flexion==null || $scope.histClinicaReh.extension==null || $scope.histClinicaReh.supinacion==null || $scope.histClinicaReh.pronacion==null))
        || ($scope.histClinicaReh.region=='6' && ($scope.histClinicaReh.flexion==null || $scope.histClinicaReh.extension==null || $scope.histClinicaReh.desvRadial==null || $scope.histClinicaReh.desvCubital==null))
        || ($scope.histClinicaReh.region=='7' && ($scope.histClinicaReh.flexion==null || $scope.histClinicaReh.extension==null || $scope.histClinicaReh.abduccion==null || $scope.histClinicaReh.aduccion==null || $scope.histClinicaReh.rotExterna==null || $scope.histClinicaReh.rotInterna==null))
        || ($scope.histClinicaReh.region=='8' && ($scope.histClinicaReh.flexion==null || $scope.histClinicaReh.extension==null || $scope.histClinicaReh.inversion==null || $scope.histClinicaReh.eversion==null))) {
/*    if ($scope.histClinicaReh.region==null || $scope.histClinicaReh.region==''  || $scope.histClinicaReh.flexion==null || $scope.histClinicaReh.extension==null ||
        $scope.histClinicaReh.lateralizacion==null || $scope.histClinicaReh.rotInterna==null ||
        $scope.histClinicaReh.rotExterna==null) {*/
/*          if ($scope.histClinicaReh.region==null) {
            $scope.error1='* Dato requerido';
          } else{
            $scope.error1='';
          };
          if ($scope.histClinicaReh.flexion==null) {
            $scope.error2='* Dato requerido';
          } else{
            $scope.error2='';
          };
          if ($scope.histClinicaReh.extension==null) {
            $scope.error3='* Dato requerido';
          } else{
            $scope.error3='';
          };
          if ($scope.histClinicaReh.lateralizacion==null) {
            $scope.error4='* Dato requerido';
          } else{
            $scope.error4='';
          };
          if ($scope.histClinicaReh.rotInterna==null) {
            $scope.error5='* Dato requerido';
          } else{
            $scope.error5='';
          };
          if ($scope.histClinicaReh.rotExterna==null) {
            $scope.error6='* Dato requerido';
          } else{
            $scope.error6='';
          };*/
          $scope.mensaje='Por favor complete los datos de la zona';
          swal({ 
            title: "Faltan datos",   
            text: $scope.mensaje,   
            type: "warning",
        ...
        if ($scope.histClinicaReh.region==10) {
          $scope.histClinicaReh.region=$scope.histClinicaReh.regionOtra;
        };
        $scope.rangos=true;
        console.log($scope.histClinicaReh);
        $http({
              url:'api/api.php?funcion=guardaRangos',
              method:'POST', 
              contentType: 'application/json', 
              dataType: "json", 
              data: $scope.histClinicaReh
                  ...
                  $http.get('api/api.php?funcion=getRangos&folio='+$rootScope.folio).success(function (data){
                      $scope.listaRangos=data;
                      console.log($scope.listaRangos);
                      $scope.histClinicaReh.region=null;
                      $scope.histClinicaReh.flexion=null;
                      $scope.histClinicaReh.extension=null;
                      $scope.histClinicaReh.lateralizacion=null;
                      $scope.histClinicaReh.rotExterna=null;
                      $scope.histClinicaReh.rotInterna=null;
                      $scope.histClinicaReh.abduccion=null;
                      $scope.histClinicaReh.aduccion=null;
                      $scope.histClinicaReh.rotaciones=null;
                      $scope.histClinicaReh.pronacion=null;
                      $scope.histClinicaReh.supinacion=null;
                      $scope.histClinicaReh.inclinaciones=null;
                      $scope.histClinicaReh.desvRadial=null;
                      $scope.histClinicaReh.desvCubital=null;
                      $scope.histClinicaReh.inversion=null;
                      $scope.histClinicaReh.eversion=null;
                      $scope.histClinicaReh.regionOtra=null;
                      $scope.rangos=false;
                      document.getElementById("formRangos").reset();
                      $('#modalRangos').modal('hide');
                  });

              }).error( function (xhr,status,data){
                $scope.mensaje ='no entra';            
                alert('Hubo un error. Preisona la tecla F5 y vuelve a intentarlo');
                $scope.rangos=false;
              });
        // $scope.rangos=false;
    };
  ...
  $scope.quitarRango = function(idRango){         
        console.log(idRango);
        $http({
              url:'api/api.php?funcion=quitarRango&idRango='+idRango,
              method:'POST', 
              contentType: 'application/json', 
              dataType: "json", 
              data: $scope.histClinicaReh
                ...
                $scope.mensaje ='no entra';            
                alert('Hubo un error. Presiona la tecla F5 y vuelve a intentarlo');
                $scope.rangos=false;
              });
  }
