app.controller('documentosCovidCtrl', function($scope,$rootScope,$location,$cookies,busquedas,$http,webStorage) {
...
$http.get('api/covid.php?funcion=getHistoria&fol='+$rootScope.folio).success(function (data){                       
  console.log(data.contador);
  if(data.contador>0){
    $scope.conHistoria=true;
  }else{
    $scope.conHistoria=false;
  }
  console.log($scope.conHistoria);
});
    ...
    $http.get('api/detallePx.php?funcion=getDatosPersonales&fol='+$rootScope.folio).success(function (data){  
                      ...
                      $scope.detalle.referencia=data.Ref_nombre;
  ...
  $http.get('api/api.php?funcion=getProducto&folio='+$rootScope.folio).success(function (data){
	...
	$scope.irHistoriaClinica = function(folio){  
        ...
        $location.path("/historiaClinicaCovid");
  ...
  $scope.irNotaMedicaMovil = function(folio){  
    ...
    $scope.enviarCorreo = function(){
      ...
      $scope.juntos = [$scope.datosEnvio, $scope.ejercicios];
          $http({
          url:'api/movil.php?funcion=envioCorreoDocs&fol='+$rootScope.folio,
          method:'POST', 
...
//validar si existe el archivo
    ...
    $scope.imprimirHistoriaClinica = function(){            
            ...
            var uri = 'api/classes/FormatoHCovid.php?fol='+$rootScope.folio;
