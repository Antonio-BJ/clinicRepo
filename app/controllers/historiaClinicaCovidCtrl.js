app.controller('historiaClinicaCovidCtrl', function($scope,$rootScope,$location,$cookies,WizardHandler,busquedas,$http,webStorage) {
  ...
  $scope.telefonos={
    ...
    $scope.arregloOpc={  
        ...
        busquedas.datosPaciente($rootScope.folio).success(function(data){            
            ...
            $scope.datos.pro = data.Pro_clave;
        
            $scope.datos.folio= $rootScope.folio;                       
            if(( ( $scope.datos.anios==null || $scope.datos.anios=='' ) || ( $scope.datos.meses==null || $scope.datos.meses=='' )) && ( $scope.datos.fecnac!=null || $scope.datos.fecnac!='' ) ){
                console.log($scope.datos.fecnac);
                $scope.calculaFecha($scope.datos.fecnac);
                // calcular edad
                // $http({
        ...
        $scope.antPersonales = function(){         
            ...
            busquedas.cirugias().success(function(data){                      
              $scope.cirugias=data;                          
            }); 
            ...
            busquedas.listaCirugias($rootScope.folio).success(function(data){                      
              if(data.length>0){   
                $scope.listaCirugias=data; 
              }                        
            });
        ...
        $scope.guardaCirugia= function(){
          if($scope.formularios.ciru.$valid){
            $scope.cargador3=true;
            console.log($scope.cirugia);
            $http({
                    url:'api/api.php?funcion=guardaCirugia&fol='+$rootScope.folio,
                    method:'POST', 
                    contentType: 'application/json', 
                    dataType: "json", 
                    data: $scope.cirugia
                     ...
                     console.log(data);
                      if(!data.respuesta){                                
                        ...
                        $scope.formularios.ciru.$submitted=false;
                      }
                      else{
                        alert('error en la inserción');
                      }
        ...
        $scope.borrarCirugia = function(idPad){ 
            $scope.cargador2=true;                             
            $http({
                    url:'api/api.php?funcion=borrarCirugia&fol='+$rootScope.folio+'&cont='+idPad,
                    method:'POST', 
                    contentType: 'application/json', 
                    dataType: "json", 
                    data: {'calve':'valor'}
        ...
        $scope.imprimirHistoriaClinica = function(motivo){          
            ...
            var uri = 'api/classes/FormatoHCovid.php?fol='+$rootScope.folio+'&motivo='+motivo+'&usr='+$rootScope.usrLogin;
         ...
         $scope.guardarDatosCovid = function(){          
              console.log($scope.covid);
              console.log($rootScope.folio);
              $scope.cargador=true;
              $http({
                      url:'api/covid.php?funcion=guardaDatosCovid&fol='+$rootScope.folio,
                      method:'POST', 
                      contentType: 'application/json', 
                      dataType: "json", 
                      data: $scope.covid
                      ...
                      console.log(data);   
                        if(data.respuesta=='exito'){
                          $scope.mensaje='Guardado Correctamente';
                          $scope.irNota='1';
                          $scope.cargador=false;
                        }else{
